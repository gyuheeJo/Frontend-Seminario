...
import getUsuarios from '../../../api/admin/consulta/consultaUsuarios.js'
...
function Usuario_lista() {
  ...
  function createData(name, lastname, code) {
  ...
  useEffect(()=>{
    getUsuarios(token).then(data => {
      if(data.success) {
        const newRows = data.data.map(user => createData(user.firstName,user.firstLastName, user.codeStudent))
        setRows(newRows)
    }}).catch(err => {
